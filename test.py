...
if __name__ == '__main__':
    ...
    parser.add_argument('--load',
                        default='./experiments/exp_1/best_model.pth', ## your best model
    ...
    parser.add_argument('--head', type=str, default='Exemplar',
                            help='choose which embedding network to use. ProtoNet, Ridge, R2D2, SVM, Subspace, Shrinkage')
    ...
    for i, batch in enumerate(tqdm(dloader_test()), 1):
        ...
        if opt.head == 'Exemplar':
            ...
            prob_log_query_sum = prob_log_query_sum.reshape(-1, opt.way)
            logit_ = prob_log_query_sum
        else:
            logit_ = F.log_softmax(logits.reshape(-1, opt.way), dim=1)

        acc = count_accuracy(logit_.reshape(-1, opt.way), labels_query.reshape(-1))
