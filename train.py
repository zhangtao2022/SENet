...
def get_model(options):
    ...
    elif options.head == 'Exemplar':
        cls_head = ClassificationHead(base_learner='Exemplar').cuda()
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', default='1') #using 4 gpus
    parser.add_argument('--network', type=str, default='ResNet',
                            help='choose which embedding network to use. ProtoNet, ResNet')
    parser.add_argument('--head', type=str, default='Exemplar',
                            help='choose which classification head to use. Subspace, Shrinkage, ProtoNet, R2D2, SVM, Exemplar')
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(dloader_train(epoch)), 1):
            ...
            if opt.head == 'Exemplar':
                exp_query = torch.exp(logit_query)
                exp_query_morn = torch.sum(exp_query, dim=(2, 3), keepdim=True)
                prob_query = torch.div(exp_query, exp_query_morn)
                prob_log_query_sum = torch.log(torch.sum(prob_query, dim=3))
                prob_log_query_sum = prob_log_query_sum.reshape(-1, opt.train_way)
                logit_ = prob_log_query_sum
            else:
                logit_ = F.log_softmax(logit_query.reshape(-1, opt.train_way), dim=1)

            loss = -(smoothed_one_hot * logit_).sum(dim=1)
            loss = loss.mean()

            acc = count_accuracy(logit_.reshape(-1, opt.train_way), labels_query.reshape(-1))
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dloader_val(epoch)), 1):
                ...
                smoothed_one_hot = one_hot(labels_query.reshape(-1), opt.test_way)
                smoothed_one_hot = smoothed_one_hot * (1 - opt.eps) + (1 - smoothed_one_hot) * opt.eps / (
                            opt.test_way - 1)
                ...
                if opt.head == 'Exemplar':
                    ...
                    prob_query = torch.div(exp_query, exp_query_morn)
                    prob_log_query_sum = torch.log(torch.sum(prob_query, dim=3))
                    prob_log_query_sum = prob_log_query_sum.reshape(-1, opt.test_way)
                    logit_ = prob_log_query_sum
                else:
                    logit_ = F.log_softmax(logit_query.reshape(-1, opt.test_way), dim=1)

                loss = -(smoothed_one_hot * logit_).sum(dim=1)
                loss = loss.mean()
                acc = count_accuracy(logit_.reshape(-1, opt.test_way), labels_query.reshape(-1))
